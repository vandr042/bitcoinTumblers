 ...
 * {@link PeerSocketHandler} which handles low-level message (de)serialization.
 * </p>
 *
 * <p>
 * Note that timeouts are handled by the extended
 * {@link org.bitcoinj.net.AbstractTimeoutHandler} and timeout is automatically
 * disabled (using
 * {@link org.bitcoinj.net.AbstractTimeoutHandler#setTimeoutEnabled(boolean)})
...
public class Peer extends PeerSocketHandler {
	 ...
	 *
	 * <p>
	 * Note that this does <b>NOT</b> make a connection to the given
	 * remoteAddress, it only creates a handler for a connection. If you want to
	 * create a one-off connection, create a Peer and pass it to
	 ...
	 *
	 * <p>
	 * The remoteAddress provided should match the remote address of the peer
	 * which is being connected to, and is used to keep track of which peers
	 * relayed transactions and offer more descriptive logging.
	 * </p>
	 */
	public Peer(NetworkParameters params, VersionMessage ver, @Nullable AbstractBlockChain chain,
	 ...
	 * <p>
	 * Construct a peer that reads/writes from the given block chain.
	 * Transactions stored in a {@link org.bitcoinj.core.TxConfidenceTable} will
	 * have their confidence levels updated when a peer announces it, to reflect
	 * the greater likelyhood that the transaction is valid.
	 * </p>
	 *
	 * <p>
	 * Note that this does <b>NOT</b> make a connection to the given
	...
	/**
	 * <p>
	 * Construct a peer that reads/writes from the given chain. Automatically
	 * creates a VersionMessage for you from the given software name/version
	 * strings, which should be something like "MySimpleTool", "1.0" and which
	 * will tell the remote node to relay transaction inv messages before it has
	 * received a filter.
	 * </p>
	 *
	 * <p>
	 ...
	 * </p>
	 */
	public Peer(NetworkParameters params, AbstractBlockChain blockChain, PeerAddress peerAddress,
	...
	/**
	 * Registers a listener that is invoked when a peer is connected or
	 * disconnected.
	 */
	...
	/**
	 * Registers a listener that is called when a transaction is broadcast
	 * across the network
	 */
	...
	/**
	 * Provides a ListenableFuture that can be used to wait for the socket to
	 * connect. A socket connection does not mean that protocol handshake has
	 * occurred.
	 */
	...
	/**
	 * <p>
	 * Returns a future that wraps a list of all transactions that the given
	 * transaction depends on, recursively. Only transactions in peers memory
	 * pools are included; the recursion stops at transactions that are in the
	 * current best chain. So it doesn't make much sense to provide a tx that
	 * was already in the best chain and a precondition checks this.
	 * </p>
	 *
	 * <p>
	 * For example, if tx has 2 inputs that connect to transactions A and B, and
	 * transaction B is unconfirmed and has one input connecting to transaction
	 * C that is unconfirmed, and transaction C connects to transaction D that
	 * is in the chain, then this method will return either {B, C} or {C, B}. No
	 * ordering is guaranteed.
	 * </p>
	 *
	 * <p>
	 * This method is useful for apps that want to learn about how long an
	 * unconfirmed transaction might take to confirm, by checking for
	 * unexpectedly time locked transactions, unusually deep dependency trees or
	 * fee-paying transactions that depend on unconfirmed free transactions.
	 * </p>
	 *
	 * <p>
	 * Note that dependencies downloaded this way will not trigger the
	 * onTransaction method of event listeners.
	 * </p>
	 */
	...
	/**
	 * Asks the connected peer for the block of the given hash, and returns a
	 * future representing the answer. If you want the block right away and
	 * don't mind waiting for it, just call .get() on the result. Your thread
	 ...
	 * Transactions in the chain cannot be retrieved this way because peers
	 * don't have a transaction ID to transaction-pos-on-disk index, and
	 * besides, in future many peers will delete old transaction data they don't
	 * need.
	 */
	@SuppressWarnings("unchecked")
	// The 'unchecked conversion' warning being suppressed here comes from the
	...
	/**
	 * Sends a getaddr request to the peer and returns a future that completes
	...
	/**
	 * When downloading the block chain, the bodies will be skipped for blocks
	 * created before the given date. Any transactions relevant to the wallet
	 * will therefore not be found, but if you know your wallet has no such
	 * transactions it doesn't matter and can save a lot of bandwidth and
	 * processing time. Note that the times of blocks isn't known until their
	 * headers are available and they are requested in chunks, so some headers
	 * may be downloaded twice using this scheme, but this optimization can
	 * still be a large win for newly created wallets.
	 *
	 * @param secondsSinceEpoch
	 *            Time in seconds since the epoch or 0 to reset to always
	...
	/**
	 * Links the given wallet to this peer. If you have multiple peers, you
	 * should use a {@link PeerGroup} to manage them and use the
	 * {@link PeerGroup#addWallet(Wallet)} method instead of registering the
	 * wallet with each peer independently, otherwise the wallet will receive
	...
	/**
	 * Starts an asynchronous download of the block chain. The chain download is
	 * deemed to be complete once we've downloaded the same number of blocks
	 * that the peer advertised having in its version handshake message.
	 */
	...
	/**
	 * Sends the peer a ping message and returns a future that will be invoked
	 * when the pong is received back. The future provides a number which is the
	 * number of milliseconds elapsed between the ping and the pong. Once the
	 * pong is received the value returned by
	 * {@link org.bitcoinj.core.Peer#getLastPingTime()} is updated.
	 * 
	...
	/**
	 * Returns the elapsed time of the last ping/pong cycle. If
	 * {@link org.bitcoinj.core.Peer#ping()} has never been called or we did not
	 * hear back the "pong" message yet, returns {@link Long#MAX_VALUE}.
	 */
	...
	/**
	 * Returns a moving average of the last N ping/pong cycles. If
	 * {@link org.bitcoinj.core.Peer#ping()} has never been called or we did not
	 * hear back the "pong" message yet, returns {@link Long#MAX_VALUE}. The
	 * moving average window is 5 buckets.
	 */
	...
	/**
	 * Returns the difference between our best chain height and the peers, which
	 * can either be positive if we are behind the peer, or negative if the peer
	 * is ahead of us.
	 */
	...
	/**
	 * Returns true if this peer will try and download things it is sent in
	 * "inv" messages. Normally you only need one peer to be downloading data.
	 * Defaults to true.
	 */
	...
	/**
	 * If set to false, the peer won't try and fetch blocks and transactions it
	 * hears about. Normally, only one peer should download missing blocks.
	 * Defaults to true. Changing this value from false to true may trigger a
	 * request to the remote peer for the contents of its memory pool, if Bloom
	 * filtering is active.
	 */
	...
	/**
	 * @return the height of the best chain as claimed by peer: sum of its ver
	 *         announcement and blocks announced since.
	 */
	...
	/**
	 * The minimum P2P protocol version that is accepted. If the peer speaks a
	 * protocol version lower than this, it will be disconnected.
	 * 
	...
	public boolean setMinProtocolVersion(int minProtocolVersion) {
		...
		if (ver != null && ver.clientVersion < minProtocolVersion) {
			log.warn("{}: Disconnecting due to new min protocol version {}, got: {}", this, minProtocolVersion,
					ver.clientVersion);
			close();
			return true;
		}
	 ...
	 * {@link BloomFilter} object to be sent to the remote peer and if either a
	 * memory pool has been set using the constructor or the vDownloadData
	 * property is true, a {@link MemoryPoolMessage} is sent as well to trigger
	 * downloading of any pending transactions that may be relevant.
	 * </p>
	 *
	 * <p>
	 * The Peer does not automatically request filters from any wallets added
	 * using {@link Peer#addWallet(Wallet)}. This is to allow callers to avoid
	 * redundantly recalculating the same filter repeatedly when using multiple
	 * peers and multiple wallets together.
	 * </p>
	 *
	 * <p>
	 * Therefore, you should not use this method if your app uses a
	 * {@link PeerGroup}. It is called for you.
	 * </p>
	 *
	 * <p>
	 * If the remote peer doesn't support Bloom filtering, then this call is
	 * ignored. Once set you presently cannot unset a filter, though the
	 * underlying p2p protocol does support it.
	 * </p>
	 */
	 ...
	 * <p>
	 * Sets a Bloom filter on this connection. This will cause the given
	 * {@link BloomFilter} object to be sent to the remote peer and if
	 * requested, a {@link MemoryPoolMessage} is sent as well to trigger
	 * downloading of any pending transactions that may be relevant.
	 * </p>
	 *
	 * <p>
	...
	/**
	 * Returns the last {@link BloomFilter} set by
	 * {@link Peer#setBloomFilter(BloomFilter)}. Bloom filters tell the remote
	 * node what transactions to send us, in a compact manner.
	 */
	...
	/**
	 * Sends a query to the remote peer asking for the unspent transaction
	 * outputs (UTXOs) for the given outpoints, with the memory pool included.
	 * The result should be treated only as a hint: it's possible for the
	 * returned outputs to be fictional and not exist in any transaction, and
	 * it's possible for them to be spent the moment after the query returns.
	 * <b>Most peers do not support this request. You will need to connect to
	 * Bitcoin XT peers if you want this to work.</b>
	 *
	 * @throws ProtocolException
	 *             if this peer doesn't support the protocol.
	 */
	...
	/**
	 * Sends a query to the remote peer asking for the unspent transaction
	 * outputs (UTXOs) for the given outpoints. The result should be treated
	 * only as a hint: it's possible for the returned outputs to be fictional
	 * and not exist in any transaction, and it's possible for them to be spent
	 * the moment after the query returns. <b>Most peers do not support this
	 * request. You will need to connect to Bitcoin XT peers if you want this to
	 * work.</b>
	 *
	 * @param includeMempool
	 *            If true (the default) the results take into account the
	 *            contents of the memory pool too.
	 * @throws ProtocolException
	 *             if this peer doesn't support the protocol.
	 */
	...
	/**
	 * Returns true if this peer will use getdata/notfound messages to walk
	 * backwards through transaction dependencies before handing the transaction
	 * off to the wallet. The wallet can do risk analysis on pending/recent
	 * transactions to try and discover if a pending tx might be at risk of
	 * double spending.
	 */
	...
	/**
	 * Sets if this peer will use getdata/notfound messages to walk backwards
	 * through transaction dependencies before handing the transaction off to
	 * the wallet. The wallet can do risk analysis on pending/recent
	 * transactions to try and discover if a pending tx might be at risk of
	 * double spending.
	 */
