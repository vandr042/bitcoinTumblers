...
def runSim(pubList, privList, privConnMap, pubConnMap, fpMap, delayModel, outBase):
    ...
    for tPriv in privList:
        for tFP in fpMap[tPriv]:
            ...
            currentTime = currentTime + random.randint(3, 500)
    frontList = []
######## Changing to cluster transactions #########
    for i in range(TX_SAMPLE_SIZE/CLUSTER_SIZE):
	...
	for j in range(CLUSTER_SIZE):
		...
		frontList.append(doTx(sendingPeer, i, privConnMap, pubConnMap, outFPs, txID, delayModel))
     #   print("**")
...
def doTx(sendingNode, clusterNum, privConn, pubConn, outFPs, txID, delayModel):
    eventMap = {}
    reachTime = {}
    lastTime = 0
        ...
        for j in range(15):
            ...
            for tPeer in toMeMap:
                if toMeMap[tPeer] < smallest:
                    ...
                    nextPeer = tPeer
	    outFPs[i].write("tx," + txID + "," + nextPeer + "," + str(smallest) + "," + str(clusterNum) + "\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cluster_size", help = "Number of transactions in a cluster", required = False, type = int, default = CLUSTER_SIZE)
    parser.add_argument("--delay_file", help = "Number of transactions in a cluster", required = False, type = str, default = DELAY_FILE)
    ...
    TX_SAMPLE_SIZE = args.sample_size * CLUSTER_SIZE
