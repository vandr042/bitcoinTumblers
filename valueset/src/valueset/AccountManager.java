...
public class AccountManager {
	...
	public void generateAliases() {
		...
		goToBlockchainExplorer(new Transaction(false, "35DafUtd6RPEi9JRK4gEc3Z6s9FRkh9ohW", 1457868000, 1.000)); // Just
																													// for
																													// temporary
																													// use
		// }
	...
	public List<Integer> runExperiment(boolean reportKeys) {
		...
		for (Transaction currentWithdrawl : this.withdrawls) {
			/*
			 * Since we assume the key is in alias reverse we need to check if
			 * it has a current alias, if it does NOT, then we mark it's alias
			 * as itself
			 */
			if(!this.aliasReverse.containsKey(currentWithdrawl.getKeyResponsible())){
				this.aliasReverse.put(currentWithdrawl.getKeyResponsible(), currentWithdrawl.getKeyResponsible());
				Set<String> dummySet = new HashSet<String>();
				dummySet.add(currentWithdrawl.getKeyResponsible());
				this.aliases.put(currentWithdrawl.getKeyResponsible(), dummySet);
			}
			...
			try {
				...
				if (!currentAnonimitySetsForAlises
						.containsKey(aliasReverse.get(currentWithdrawl.getKeyResponsible()))) {
					currentAnonimitySetsForAlises.put(aliasReverse.get(currentWithdrawl.getKeyResponsible()), anonSet);
				}
				intersection.retainAll(
						currentAnonimitySetsForAlises.get(aliasReverse.get(currentWithdrawl.getKeyResponsible())));
				currentAnonimitySetsForAlises.replace(aliasReverse.get(currentWithdrawl.getKeyResponsible()),
						intersection);
	...
	private Set<String> getAnonimitySet(Transaction withdrawl) {
		...
		double summedWithdrawalValues = 0;
		...
		/*
		 * Now we increase summedWithdrawalValues if an alias made a withdrawl
		 * earlier
		 */
		for (Transaction tWithdrawal : this.withdrawls) {
			...
			// Check if this withdrawal is in our current withdrawal's anonymity
			// set
			if (aliasReverse.containsKey(withdrawl.getKeyResponsible())) {
				if (aliases.get(aliasReverse.get(withdrawl.getKeyResponsible()))
						.contains(tWithdrawal.getKeyResponsible())) {
					// If it is, then increase the value of the actual
					// withdrawal
